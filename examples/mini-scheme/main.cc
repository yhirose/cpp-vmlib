...
void usage() {
  std::cerr << "usage: mini-scheme [--dump-ir] [--dump-bc] PROGRAM.scm\n";
}
...
int main(int argc, char** argv) {
  ...
  if (!in) {
    std::cerr << "mini-scheme: cannot open " << path << "\n";
    return 2;
  }
  ...
  const coreir::Module m = mini_scheme::bind_source(source);
  ...
  vm::RunOptions opts;
  opts.natives = mini_scheme::stdlib();
