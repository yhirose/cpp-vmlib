...
void usage() {
  std::cerr << "usage: mini-csharp [--dump-ir] [--dump-bc] PROGRAM.cs\n";
}
...
int main(int argc, char** argv) {
  ...
  if (!in) {
    std::cerr << "mini-csharp: cannot open " << path << "\n";
    return 2;
  }
  ...
  const coreir::Module m = mini_csharp::bind_source(source);
  ...
  vm::RunOptions opts;
  opts.natives = mini_csharp::stdlib();
